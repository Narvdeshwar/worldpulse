...
import { Settings, Check, Palette, Sun, Moon } from "lucide-react";
...
export function ThemeSwitcher() {
  ...
  useEffect(() => {
    try {
      const savedTheme = localStorage.getItem("wp-theme") || "emerald";
      const savedMode = localStorage.getItem("wp-mode") === "dark";
  ...
  const toggleMode = () => {
    ...
    if (newMode) {
      document.documentElement.classList.add("dark");
      localStorage.setItem("wp-mode", "dark");
    } else {
      document.documentElement.classList.remove("dark");
      localStorage.setItem("wp-mode", "light");
    }
  };
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-52 bg-card/95 backdrop-blur-2xl border-primary/20 p-2 rounded-xl shadow-xl">
        ...
        <div className="px-1 pb-1">
          <button
            onClick={toggleMode}
            className="flex items-center justify-between w-full px-3 py-2.5 rounded-lg bg-primary/5 hover:bg-primary/10 border border-primary/10 transition-all group"
          >
            ...
            <div className={cn(
              ...
              isDark ? "bg-primary/40" : "bg-muted"
            )}>
              <div className={cn(
                "absolute top-1 w-2 h-2 rounded-full transition-all duration-300",
                isDark ? "right-1 bg-primary shadow-[0_0_8px_var(--primary-glow)]" : "left-1 bg-background"
              )} />
            </div>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 gap-1 pt-1">
          {themes.map((theme) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className={cn("h-3 w-3 rounded-full shadow-[0_0_10px_rgba(0,0,0,0.1)]", theme.color)} />
